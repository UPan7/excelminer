...
const Index = () => {
  ...
  const loadAvailableMetals = async () => {
    try {
      console.log('Loading available metals from database...');
      ...
      const rawMetals = data?.map(item => item.metal) || [];
      console.log('Raw metals from DB:', rawMetals.length, 'items');
      
      const uniqueMetals = [...new Set(rawMetals)]
        .filter(metal => metal != null && metal.trim() !== '')
        .sort();

      console.log('Processed unique metals:', uniqueMetals.length, 'items:', uniqueMetals);
